...
class FixedDiscountPolicy implements DiscountPolicy {
    ...
    public Money calculateDiscount(Money originalPrice, Long discountValue) {
        requireNonNull(originalPrice, "FixedDiscountPolicy.calculateDiscount().originalPrice: 원가는 null일 수 없습니다.");
        requirePositive(discountValue, "FixedDiscountPolicy.calculateDiscount().discountValue: 할인 금액은 양수여야 합니다.");
